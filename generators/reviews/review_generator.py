...
SCORE_DIST = {key : value for key, 
            value in raw_score_dist.items()
            if key in ["1", "2", "3", "4", "5"]}
# Normalize weights

total_weight = sum(SCORE_DIST.values())
...
def generate_review(order: dict) -> dict:
    ...
    """
    score = int(weighted_choice(SCORE_DIST))
    if score >= 4:
    ...
    delivered_dt = datetime.strptime(delivered_date_str, "%Y-%m-%d %H:%M:%S")
    
    creation_dt = delivered_dt + timedelta(days=random.randint(1, 3))
    answer_dt = creation_dt + timedelta(days=random.randint(1, 4))
