...
def generate_customer_location(customer: dict) -> dict:
    ...
    if city not in city_coordinate_map:
        ...
        valid_cities = [
            cities for cities in state_cities if cities in city_coordinate_map
        ]
        if valid_cities:
    ...
    return {
        ...
        "geolocation_lat": coords["geolocation_lat"] + random.uniform(-jitter, jitter),
        "geolocation_lng": coords["geolocation_lng"] + random.uniform(-jitter, jitter),
    }
