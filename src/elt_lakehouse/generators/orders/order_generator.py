...
def generate_estimated_delivery_date(purchase_timestamp: datetime) -> datetime:
    delay: float = float(weighted_choice(ORDER_ESTIMATED_DELIVERY_DELAY_DIST))
...
def generate_order() -> dict:
    ...
    order_status = weighted_choice(ORDER_STATUS_DIST)

    purchase_timestamp = generate_order_purchase_timestamp()
    ...
    if order_status in GEN_CONFIG_STATUS["shipped_or_later"]:
        ...
        delivered_carrier_date = generate_delivered_carrier_date(approved_at)

    if order_status in GEN_CONFIG_STATUS["delivered"]:

        delivered_customer_date = generate_delivered_customer_date(
            delivered_carrier_date
        )

    if order_status in GEN_CONFIG_STATUS["unavailable"]:
        approved_at = generate_approved_timestamp(purchase_timestamp)
