...
def build_order_profiles() -> None:
    ...
    order_status_distribution = (
        order["order_status"].str.strip().value_counts(normalize=True).to_dict()
    )
    ...
    order["order_purchase_timestamp"] = pd.to_datetime(
        order["order_purchase_timestamp"], errors="coerce"
    )
    order["order_delivered_customer_date"] = pd.to_datetime(
        order["order_delivered_customer_date"], errors="coerce"
    )
    delivery_days = (
    ...
    carrier_delay = carrier_delay[carrier_delay >= 0]

    carrier_distribution = carrier_delay.value_counts().sort_index().round(2).to_dict()
    ...
    # ==================================
    delivery_delay = (
        order["order_delivered_customer_date"] - order["order_delivered_carrier_date"]
    ).dt.days.dropna()

    delivery_delay = delivery_delay[delivery_delay >= 0]

    delivery_delay_distribution = (
        delivery_delay.value_counts(normalize=True).sort_index().round(4).to_dict()
    )
    ...
    total_delivery_delay_distribution = (
        Total_delivery_delay.value_counts(normalize=True)
        .sort_index()
        .round(4)
        .to_dict()
    )
    ...
    estimated_delivery_delay_distribution = (
        estimated_delivery_delay.value_counts(normalize=True)
        .sort_index()
        .round(4)
        .to_dict()
    )
    ...
    # =========================================
    merged = order.merge(order_items, on="order_id")
